...
as $$
begin 
    insert into btc_wallet_money_flow(wallet_id, num_address, category, total_amount, flow_type) (
      select wallet_id, count(distinct address) as num_address, category, SUM(tx_value) as total_amount, 'IN' as flow_type from (
      select cluster_id as wallet_id, cluster_address.address, category, tx_value from btc_tx_input join (
...
begin 
    insert into btc_wallet_money_flow(wallet_id, num_address, category, total_amount, flow_type) (
      select wallet_id, count(distinct address) as num_address, category, SUM(tx_value) as total_amount, 'OUT' as flow_type from (
      select cluster_id as wallet_id, cluster_address.address, category, tx_value from btc_tx_output join (
      select cluster_id, btc_address_cluster.address, category from btc_address_cluster 
      join btc_address_label on btc_address_cluster.address = btc_address_label.address
